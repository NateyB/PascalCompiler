\documentclass{article}
\usepackage[margin=1in]{geometry}
\usepackage{array}
\usepackage{bm}

\pagenumbering{gobble}

%chktex-file 9
%chktex-file 26
%chktex-file 37

\newenvironment{productions}[0]{
    \let\oldepsilon\epsilon{}
    \renewcommand{\epsilon}[0]{\bm{\oldepsilon{}}}
    \newcommand{\tm}[1]{\textbf{\emph{##1}}} % Short for term
    \newcolumntype{R}[1]{>{\raggedleft\hspace{0pt}}m{##1}}
    \newcommand{\production}[3]
    {
        ##1. & \emph{##2} & $\rightarrow{}$ & \emph{\begin{tabular}[t]{l}
                                                    ##3
                                                \end{tabular}} \\
    }
    \begin{tabular}{R{1cm}R{4cm}cl}
}{
    \end{tabular}
}

\begin{document}
    \begin{productions}
        \production{1}{program}{\tm{program id} \tm{(} identifier\_list
                                                                \tm{)  ;} \\
                                declarations \\
                                subprogram\_declarations \\
                                compound\_statement \\
                                \tm{.}
                                }
        \production{2.1}{identifier\_list}{\tm{id}}
        \production{2.2}{identifier\_list}{identifier\_list , \tm{id}}
        \production{3.1}{declarations}{declarations \tm{var} identifier\_list
                                                               : type ;}
        \production{3.2}{declarations}{$\epsilon{}$}
        \production{4.1}{type}{standard\_type}
        \production{4.2}{type}{\tm{array [ num ..\ num ] of} standard\_type}
        \production{5.1}{standard\_type}{\tm{integer}}
        \production{5.2}{standard\_type}{\tm{real}}

        \production{6.1}{subprogram\_declarations}{subprogram\_declarations
                                                   subprogram\_declaration
                                                   \tm{;}}
        \production{6.2}{subprogram\_declarations}{$\epsilon{}$}
        \production{7}{subprogram\_declaration}{subprogram\_head declarations
                                                compound\_statement}
        \production{8.1}{subprogram\_head}{\tm{function id} arguments
                                           \tm{:} standard\_type \tm{;}}
        \production{8.2}{subprogram\_head}{\tm{procedure id} arguments \tm{;}}
        \production{9.1}{arguments}{\tm{(} parameter\_list \tm{)}}
        \production{9.2}{arguments}{$\epsilon{}$}
        \production{10.1}{parameter\_list}{identifier\_list \tm{:} type}
        \production{10.2}{parameter\_list}{parameter\_list \tm{;}
                                           identifier\_list \tm{:} type}

        \production{11}{compound\_statement}{\tm{begin} \\
                                             optional\_statements \\
                                             \tm{end}}
        \production{12.1}{optional\_statements}{statement\_list}
        \production{12.2}{optional\_statements}{$\epsilon{}$}
        \production{13.1}{statement\_list}{statement}
        \production{13.2}{statement\_list}{statement\_list \tm{;} statement}
        \production{14.1}{statement}{variable \tm{assignop} expression}
        \production{14.2}{statement}{procedure\_statement}
        \production{14.3}{statement}{compound\_statement}
        \production{14.4}{statement}{\tm{if} expression \tm{then} statement
                                     \tm{else} statement}
        \production{14.5}{statement}{\tm{while} expression \tm{do} statement}

        \production{15.1}{variable}{\tm{id}}
        \production{15.2}{variable}{\tm{id [} expression \tm{]}}
        \production{16.1}{procedure\_statement}{\tm{id}}
        \production{16.2}{procedure\_statement}{\tm{id (} expression\_list
                                                \tm{)}}
        \production{17.1}{expression\_list}{expression}
        \production{17.2}{expression\_list}{expression\_list \tm{,} expression}
        \production{18.1}{expression}{simple\_expression}
        \production{18.2}{expression}{simple\_expression \tm{relop}
                                      simple\_expression}
        \production{19.1}{simple\_expression}{term}
        \production{19.2}{simple\_expression}{sign term}
        \production{19.3}{simple\_expression}{simple\_expression \tm{addop}
                                              term}

        \production{20.1}{term}{factor}
        \production{20.2}{term}{term \tm{mulop} factor}
        \production{21.1}{factor}{\tm{id}}
        \production{21.2}{factor}{\tm{id (} expression\_list \tm{)}}
        \production{21.3}{factor}{\tm{num}}
        \production{21.4}{factor}{\tm{(} expression \tm{)}}
        \production{21.5}{factor}{\tm{not} factor}
        \production{22.1}{sign}{\bm{$+$}}
        \production{22.2}{sign}{\bm{$-$}}


    \end{productions}

\end{document}
